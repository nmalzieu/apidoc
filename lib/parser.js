...
Parser.prototype._findBlocks = function()
{
	...
	var docBlocksRegExpJs = /\/\*\*\uffff(.+?)\*\//g;
	var matches = docBlocksRegExpJs.exec(src);
	while(matches)
	{
		var block = matches[1];
		...
		matches = docBlocksRegExpJs.exec(src);
	} // while
	var docBlocksRegExpCoffee = /###\*\*\uffff(.+?)\*\*###/g;
	var matches1 = docBlocksRegExpCoffee.exec(src);
	while(matches1)
	{
		var block = matches1[1];
		...
		// Nächsten Eintrag finden
		matches1 = docBlocksRegExpCoffee.exec(src);
	} // while
	return blocks;
}; // _findBlocks
